...
class RepBlock(nn.Module):
    def __init__(self, inp_planes, out_planes, feature_size=256, mode='train', act_type = 'linear', with_idt = True):
        ...
        if with_idt and (self.inp_planes == self.out_planes):
            ...
            # residual = np.zeros([self.kx, self.ky, self.inp_planes, self.out_planes], dtype=np.float32)
            # if self.with_idt:
            #     mid_kx = int(self.kx / 2)
            #     mid_ky = int(self.ky / 2)

            #     for out_ch in range(self.out_planes):
            #         residual[mid_kx, mid_ky, :, out_ch] = 1.0
...
if __name__ == '__main__':
    ...
    ecb = RepBlock(inp_planes=3, out_planes=3, feature_size=256, mode='train', act_type= 'linear', with_idt = False).cuda().eval()
